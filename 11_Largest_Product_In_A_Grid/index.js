function getLargestProductInGrid(grid, numsLen) {
  ...
  for (let row = 0; row < gridLen; row++) {
    for (let col = 0; col < gridLen; col++) {
      ...
      for (let i = 0; i < numsLen; i++) {
        horizontal *= getNum(grid, row, col + i);
        vertical *= getNum(grid, row + i, col);
        rightDiag *= getNum(grid, row + i, col + i);
        leftDiag *= getNum(grid, row + i, col - i);
      }
...
console.log(getLargestProductInGrid(grid, 4));
